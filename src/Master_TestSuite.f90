...
!*                                                   *
!*   The gradient test is performed in a separate    *
!*   routine (Master_GradTest.f90)                   *
!*                                                   *
!*                                                   *
!*****************************************************
...
TYPE(ABC_type), ALLOCATABLE :: Model_data_array1(:), Model_data_array2(:)
TYPE(ABC_type), ALLOCATABLE :: LSarray(:)
TYPE(CVT_type)              :: CVT
REAL(ZREAL8),   ALLOCATABLE :: Field1(:,:)
REAL(ZREAL8),   ALLOCATABLE :: Field1a(:,:)
REAL(ZREAL8),   ALLOCATABLE :: Field2(:,:)
REAL(ZREAL8),   ALLOCATABLE :: Field2a(:,:)
REAL(ZREAL8),   ALLOCATABLE :: Field3(:,:)
REAL(ZREAL8),   ALLOCATABLE :: Field3a(:,:)
REAL(ZREAL8)                :: TwoPoints1(1:2), TwoPoints2(1:2)
REAL(ZREAL8)                :: AxisValuesx(1:2), AxisValuest(1:2), ObValue
REAL(ZREAL8)                :: Value
REAL(ZREAL8)                :: LHS, RHS
REAL(ZREAL8),   ALLOCATABLE :: testmatrix(:,:)
...
IF (RunAdjTests_CVT) THEN
  ...
  CALL Initialise_model_vars (Pert2, .FALSE.)
  CALL Initialise_model_vars (Pert3, .FALSE.)
  ALLOCATE (Field1(0:nlongs+1,0:nlevs+1))
  ALLOCATE (Field1a(0:nlongs+1,0:nlevs+1))
  ALLOCATE (Field2(0:nlongs+1,0:nlevs+1))
  ALLOCATE (Field2a(0:nlongs+1,0:nlevs+1))
  ALLOCATE (Field3(0:nlongs+1,0:nlevs+1))
  ALLOCATE (Field3a(0:nlongs+1,0:nlevs+1))
  ...
  CALL RANDOM_NUMBER (Field1a(0:nlongs+1,0:nlevs+1))                           ! v

  ! Act with forward operator
  CALL Anbalw (LS,                                 &
               Field1(0:nlongs+1,0:nlevs+1),       &
               Field1a(0:nlongs+1,0:nlevs+1),      &
  ...
  CALL Anbalw_adj (LS,                             &
                   Field3(0:nlongs+1,0:nlevs+1),   &                           ! A^T A v
                   Field3a(0:nlongs+1,0:nlevs+1),  &                           ! A^T A v
                   Field2(1:nlongs,1:nlevs), dims)
  ! Compute the RHS
  RHS = SUM(Field3(0:nlongs+1,0:nlevs+1) * Field1(0:nlongs+1,0:nlevs+1)) + &
        SUM(Field3a(0:nlongs+1,0:nlevs+1) * Field1a(0:nlongs+1,0:nlevs+1))     ! (A^T A v)^T v
  ...
  CALL Deallocate_CVs (CV_data1)
  CALL Deallocate_CVs (CV_data2)
  CALL Deallocate_CVs (CV_data3)
  CALL Deallocate_model_vars (Pert2)
  CALL Deallocate_model_vars (Pert3)
  DEALLOCATE (Field1, Field1a, Field2, Field2a, Field3, Field3a)
...
IF (RunAdjTests_obs) THEN
  ...
  ALLOCATE (Model_data_array1(1:2))
  CALL Initialise_model_vars (Model_data_array1(1), .FALSE.)
  CALL Initialise_model_vars (Model_data_array1(2), .FALSE.)
  ALLOCATE (Model_data_array2(1:2))
  ...
  CALL Read_state_2d (LS_filename, LSfc(0), dims, 1, .TRUE.)
  PRINT*, '-- done'
  ...
  DO t = 0, DAtimestepsreq
    CALL Initialise_model_vars (ABC1(t), .FALSE.)
  END DO

  Pert_filename = TRIM(datadirABCperts) // '/' // TRIM(Pert_file)
  PRINT *, 'The file containing the perts: ', TRIM(Pert_filename)
  PRINT*, 'Reading in perturbations ...'
  CALL Read_state_2d (Pert_filename, ABC1(0), dims, 1, .TRUE.)
  PRINT*, '-- done'
  ...
  ! Dealocate
  DO t = 0, DAtimestepsreq
    CALL Deallocate_model_vars (ABC1(t))
    CALL Deallocate_model_vars (ABC2(t))
  END DO
  ...
  ! Tidy up
  CALL Deallocate_model_vars (Model_data_array1(1))
  CALL Deallocate_model_vars (Model_data_array1(2))
  DEALLOCATE (Model_data_array1)
  CALL Deallocate_model_vars (Model_data_array2(1))
  CALL Deallocate_model_vars (Model_data_array2(2))
  DEALLOCATE (Model_data_array2)
...
IF (RunInvTests) THEN
  ...
  CALL Initialise_model_vars (Pert1, .FALSE.)
  CALL Initialise_model_vars (Pert2, .FALSE.)
  CALL Initialise_CVs (CV_data1, .FALSE.)
  ALLOCATE (testmatrix(1:nlevs,1:nlevs))
  ALLOCATE (check(1:nlevs,1:nlevs))
  ...
  CALL Read_state_2d (Pert_filename, Pert1, dims, 1, .TRUE.)
  PRINT*, '-- done'
  ...
  CALL Write_state_2d (InvTestOutput_filename, Pert2, dims, 1, 0, 0, .TRUE.)

  ! Tidy up
  CALL Deallocate_model_vars (Pert1)
  CALL Deallocate_model_vars (Pert2)
  CALL Deallocate_CVs (CV_data1)
  DEALLOCATE (testmatrix, check)
...
IF (RunAdjTests_CVT .OR. RunInvTests) CALL Deallocate_CVT (CVT)
