  ...
  DO batch = 1, ObsSpec % NumBatches
    DO x = 1, ObsSpec % NumObs_long(batch)
      ...
      DO z = 1, ObsSpec % NumObs_height(batch)
        ...
        WRITE (13,'(A18,F12.3)') 'Longitude (m)   : ', longitude
...
CASE (2)
  ...
  DO
    ...
    ELSE
      ...
      READ (13,'(A18,F12.3)') blank, thisob % longitude_m
      READ (13,'(A18,F12.3)') blank, thisob % level_ht
      READ (13,'(A18,I3)')    blank, thisob % ob_of_what
      READ (13,'(A18,F12.8)') blank, thisob % stddev
      PRINT '(I10,I6,I8,F12.3,F12.3,I3,F12.3)', thisob % obnumber_thisfile, thisob % batch, thisob % t, thisob % longitude_m, &
  ...
  DO t = 0, DAtimestepsreq
    CALL Initialise_model_vars (ABC(t), .FALSE.)
  END DO
  ...
  CALL Read_state_2d (ABC_init_filename, ABC(0), dims, -1, .TRUE.)
  PRINT*, '-- done'
  ...
  DO t = 0, DAtimestepsreq
    ...
    CALL Write_state_2d (Truth_filename, ABC(t), dims, DAtimestepsreq+1, t, model_steps_per_da_step, .TRUE.)
  END DO
  ...
  DO t = 0, DAtimestepsreq
    CALL Deallocate_model_vars (ABC(t))
  END DO
  DEALLOCATE (ABC)
...
CASE (3)
  ...
  CALL Initialise_model_vars (ABC(2), .FALSE.)
  CALL Initialise_model_vars (ABC(3), .FALSE.)
  ...
  CALL Read_state_2d (ABC_init_filename, ABC(1), dims, -1, .TRUE.)
  PRINT*, '-- done'
  ...
  IF (Bg_inflation /= 0.0) THEN
    ! Inflate the CVs
    CV_data % v1(1:nlongs,1:nlevs) = CV_data % v1(1:nlongs,1:nlevs) * Bg_inflation
    CV_data % v2(1:nlongs,1:nlevs) = CV_data % v2(1:nlongs,1:nlevs) * Bg_inflation
    CV_data % v3(1:nlongs,1:nlevs) = CV_data % v3(1:nlongs,1:nlevs) * Bg_inflation
    CV_data % v4(1:nlongs,1:nlevs) = CV_data % v4(1:nlongs,1:nlevs) * Bg_inflation
    CV_data % v5(1:nlongs,1:nlevs) = CV_data % v5(1:nlongs,1:nlevs) * Bg_inflation
    CV_data % v6(1:nlongs,1:nlevs) = CV_data % v6(1:nlongs,1:nlevs) * Bg_inflation
  END IF

  ! Act with the forward CVT to generate the perturbation
  CALL U_trans (ABC(1), CV_data, ABC(2), CVT, dims)
  ! Update rho increment
  ABC(2) % rho(0:nlongs+1,0:nlevs+1) = ABC(2) % r(0:nlongs+1,0:nlevs+1)
  ...
  CALL Write_state_2d (Pert_filename, ABC(2), dims, 1, 0, 0, .TRUE.)

  ! The background filename
  Bg_filename = TRIM(datadir_Bg) // '/' // TRIM(Bg_file)
  PRINT *, 'The bg pert output file: ', TRIM(Bg_filename)
  CALL Write_state_2d (Bg_filename, ABC(3), dims, 1, 0, 0, .TRUE.)

  CALL Deallocate_model_vars (ABC(1))
  CALL Deallocate_model_vars (ABC(2))
  CALL Deallocate_model_vars (ABC(3))
  DEALLOCATE (ABC)
