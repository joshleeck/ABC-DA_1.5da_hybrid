...
DO
  IF (ASSOCIATED(thisob)) THEN
    !PRINT *
    !PRINT '(I6,I8,F12.3,F12.3,I3,F12.3)', thisob % batch, thisob % t, thisob % longitude_m, &
    ...
    IF ((thisob % tstep_lower .GE. 0) .AND. (thisob % tstep_lower .LE. steps)) THEN
      ...
      IF (thisob % xbox_lower == -1) THEN  ! If not already known
        ...
        CASE (1) ! u longitudes
          thisob % xbox_lower    = FindLowerIndex(thisob % longitude_m, nlongs+2, dims % longs_u(0:nlongs+1))
          thisob % xbox_lower_ws = 0
        CASE (2,3,4,5,6) ! v longitudes
          thisob % xbox_lower    = FindLowerIndex(thisob % longitude_m, nlongs+2, dims % longs_v(0:nlongs+1))
          thisob % xbox_lower_ws = 0
        CASE (7,8) !Wind speed, so u and v information required
          thisob % xbox_lower    = FindLowerIndex(thisob % longitude_m, nlongs+2, dims % longs_u(0:nlongs+1))
          thisob % xbox_lower_ws = FindLowerIndex(thisob % longitude_m, nlongs+2, dims % longs_v(0:nlongs+1))
        END SELECT
      END IF
