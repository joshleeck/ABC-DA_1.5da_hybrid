SUBROUTINE U_h_inv (ControlVar1, ControlVar2, CVT, fftonly_flag)

! Code to perform the inverse horizontal cvt: ControlVar1 = U_h^-1 ControlVar2
...
LOGICAL,        INTENT(IN)    :: fftonly_flag
...
IF (.NOT.fftonly_flag) THEN

  ! Divide by the evs (these are actually the square-roots)
...
! Do non-symmetric transform - keep in spectral space
