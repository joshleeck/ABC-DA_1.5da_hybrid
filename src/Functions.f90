...
GAUSS = SQRT(-2.0*LOG(u1)) * COS(2.0*pi*u2) * std;
...
!=================================================================================================
REAL(ZREAL8) FUNCTION Interpolate2D (Field, axisx, axisy, x, y)
...
REAL(ZREAL8), INTENT(IN) :: Field(1:2, 1:2)   ! The main field to be interpolated
REAL(ZREAL8), INTENT(IN) :: axisx(1:2)        ! The x axis values
REAL(ZREAL8), INTENT(IN) :: axisy(1:2)        ! The y axis values
REAL(ZREAL8), INTENT(IN) :: x                 ! The x point to interpolate to
REAL(ZREAL8), INTENT(IN) :: y                 ! The y point to interpolate to
...
ys(1) = Interpolate1D (Field(1:2,1), axisx(1:2), x)
ys(2) = Interpolate1D (Field(1:2,2), axisx(1:2), x)

! Initerpolate in y
Interpolate2D = Interpolate1D (ys(1:2), axisy(1:2), y)
