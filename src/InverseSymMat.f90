...
!---------------------
INTEGER                     :: ierr, i, j
REAL(ZREAL8), ALLOCATABLE   :: evecs(:,:)
REAL(ZREAL8), ALLOCATABLE   :: evals(:)
REAL(ZREAL8), ALLOCATABLE   :: Work(:)
REAL(ZREAL8)                :: totaleigs, add, inv
REAL(ZREAL8), ALLOCATABLE   :: invlambdaFT(:,:)


ALLOCATE (evecs(1:dim,1:dim))
ALLOCATE (evals(1:dim))
ALLOCATE (Work(1:3*dim))
ALLOCATE (invlambdaFT(1:dim,1:dim))
...
DEALLOCATE (evecs, evals, Work, invlambdaFT)
