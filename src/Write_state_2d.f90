SUBROUTINE Write_state_2d (filename, state, dims, ntimes, t, wts, core_only)
...
IF (t .EQ. 0) THEN
  ...
  ELSE
    ...
    ddC(2)  = dimidHalf_levs
    ddC(3)  = dimid_time
    ierr10b = NF_DEF_VAR(ncid, 'horiz_div', NF_DOUBLE, 3, ddC, varid_horizdiv)

    ddC(1)  = dimidLongs_u
    ddC(2)  = dimidHalf_levs
    ddC(3)  = dimid_time
    ierr10c = NF_DEF_VAR(ncid, 'horiz_vort', NF_DOUBLE, 3, ddC, varid_horizvort)
