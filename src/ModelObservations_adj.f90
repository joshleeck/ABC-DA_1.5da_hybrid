...
!*********************************************************************************
!*                                                                               *
!*  Adj of: Compute model's version of observations based on pre-run model states*
!*  (perturbations are adjoint states)                                           *
!*                                                                               *
...
!*  Operate_On             - 1 to operate with adjoint on deltay_m_hat           *
!*                           (this applies to gradient calculations)             *
!*                         - 2 to operate with adjoint on deltay_m               *
!*                           (this applies to adjoint test calculations)         *
!*                                                                               *
...
!PRINT *, 'Inside ModelObservations_adj'
!PRINT *, '----------------------------'
...
DO
  IF (ASSOCIATED(thisob)) THEN
    IF (thisob % ob_ok) THEN
      ...
      IF (OperateOn == 1) THEN
        d_prelim_yval = thisob % deltay_m_hat
      ELSE
        d_prelim_yval = thisob % deltay_m
      END IF
      ...
      CASE(1)  ! Zonal wind
      !--------------------
        CALL Interpolate3D_adj (d_prelim_yval,                                                             &
                                tsteps_needed4_thisob,                                                     &
      ...
      !-------------------------------
        ...
        d_prelim_yval_u = prelim_yval_u * d_prelim_yval / thisob % y_ref
        d_prelim_yval_v = prelim_yval_v * d_prelim_yval / thisob % y_ref


        CALL Interpolate3D_adj (d_prelim_yval_u,                                                           &
                                tsteps_needed4_thisob,                                                     &
                                PertStates(thisob % tstep_lower:thisob % tstep_lower+tsteps_needed4_thisob-1),&
                                dims % longs_u(thisob % xbox_lower:thisob % xbox_lower+1),                 &
                                dims % half_levs(thisob % zbox_lower:thisob % zbox_lower+1),               &
                                ...
                                thisob % t,                                                                &
                                1)
        CALL Interpolate3D_adj (d_prelim_yval_v,                                                           &
                                tsteps_needed4_thisob,                                                     &
                                PertStates(thisob % tstep_lower:thisob % tstep_lower+tsteps_needed4_thisob-1),&
                                dims % longs_v(thisob % xbox_lower_ws:thisob % xbox_lower_ws+1),           &
                                dims % half_levs(thisob % zbox_lower:thisob % zbox_lower+1),               &
      ...
      !--------------------------
        ...
        d_prelim_yval_u = prelim_yval_u * d_prelim_yval / thisob % y_ref
        d_prelim_yval_v = prelim_yval_v * d_prelim_yval / thisob % y_ref
        d_prelim_yval_w = prelim_yval_w * d_prelim_yval / thisob % y_ref
        ...
        CALL Interpolate3D_adj (d_prelim_yval_v,                                                           &
                                ...
                                thisob % t,                                                                &
                                2)
        CALL Interpolate3D_adj (d_prelim_yval_w,                                                           &
                                tsteps_needed4_thisob,                                                     &
                                PertStates(thisob % tstep_lower:thisob % tstep_lower+tsteps_needed4_thisob-1),&
                                dims % longs_v(thisob % xbox_lower_ws:thisob % xbox_lower_ws+1),           &
                                dims % full_levs(thisob % zbox_lower_ws:thisob % zbox_lower_ws+1),         &
