...
INTEGER                         :: k
INTEGER                         :: real_index, imag_index, nover2p1
...
IF (DABS(div_cons) < small) THEN
  ...
  CVT % HorizEV1(nover2p1,1:nlevs) = CVT % HorizEV1(nover2p1,1:nlevs) + &
                                     pert % v1(nlongs,1:nlevs) * pert % v1(nlongs,1:nlevs)
  CVT % HorizEV2(nover2p1,1:nlevs) = CVT % HorizEV2(nover2p1,1:nlevs) + &
                                     pert % v2(nlongs,1:nlevs) * pert % v2(nlongs,1:nlevs)
  CVT % HorizEV3(nover2p1,1:nlevs) = CVT % HorizEV3(nover2p1,1:nlevs) + &
                                     pert % v3(nlongs,1:nlevs) * pert % v3(nlongs,1:nlevs)
  CVT % HorizEV4(nover2p1,1:nlevs) = CVT % HorizEV4(nover2p1,1:nlevs) + &
                                     pert % v4(nlongs,1:nlevs) * pert % v4(nlongs,1:nlevs)
  CVT % HorizEV5(nover2p1,1:nlevs) = CVT % HorizEV5(nover2p1,1:nlevs) + &
                                     pert % v5(nlongs,1:nlevs) * pert % v5(nlongs,1:nlevs)
  CVT % HorizEV6(nover2p1,1:nlevs) = CVT % HorizEV6(nover2p1,1:nlevs) + &
...
ELSE
  ...
  ! Normalize (recall that twice as many states went into determining all but the largest and
  ! smallest scales, the two states being k and -k)
  ! Deal with the largest scale
  mul_cons = 1.0 / div_cons
  CVT % HorizEV1(1,1:nlevs) = CVT % HorizEV1(1,1:nlevs) * mul_cons
  CVT % HorizEV2(1,1:nlevs) = CVT % HorizEV2(1,1:nlevs) * mul_cons
  CVT % HorizEV3(1,1:nlevs) = CVT % HorizEV3(1,1:nlevs) * mul_cons
  CVT % HorizEV4(1,1:nlevs) = CVT % HorizEV4(1,1:nlevs) * mul_cons
  CVT % HorizEV5(1,1:nlevs) = CVT % HorizEV5(1,1:nlevs) * mul_cons
  CVT % HorizEV6(1,1:nlevs) = CVT % HorizEV6(1,1:nlevs) * mul_cons

  ! Deal with the intermediate scales
  mul_cons = 0.5 / div_cons
  DO k = 2, nlongs/2
    CVT % HorizEV1(k,1:nlevs) = CVT % HorizEV1(k,1:nlevs) * mul_cons
    CVT % HorizEV2(k,1:nlevs) = CVT % HorizEV2(k,1:nlevs) * mul_cons
    CVT % HorizEV3(k,1:nlevs) = CVT % HorizEV3(k,1:nlevs) * mul_cons
    CVT % HorizEV4(k,1:nlevs) = CVT % HorizEV4(k,1:nlevs) * mul_cons
    CVT % HorizEV5(k,1:nlevs) = CVT % HorizEV5(k,1:nlevs) * mul_cons
    CVT % HorizEV6(k,1:nlevs) = CVT % HorizEV6(k,1:nlevs) * mul_cons
  END DO

  ! Deal with the smallest scale
  mul_cons = 1.0 / div_cons
  CVT % HorizEV1(nover2p1,1:nlevs) = CVT % HorizEV1(nover2p1,1:nlevs) * mul_cons
  CVT % HorizEV2(nover2p1,1:nlevs) = CVT % HorizEV2(nover2p1,1:nlevs) * mul_cons
  CVT % HorizEV3(nover2p1,1:nlevs) = CVT % HorizEV3(nover2p1,1:nlevs) * mul_cons
  CVT % HorizEV4(nover2p1,1:nlevs) = CVT % HorizEV4(nover2p1,1:nlevs) * mul_cons
  CVT % HorizEV5(nover2p1,1:nlevs) = CVT % HorizEV5(nover2p1,1:nlevs) * mul_cons
  CVT % HorizEV6(nover2p1,1:nlevs) = CVT % HorizEV6(nover2p1,1:nlevs) * mul_cons



  DO k = 1, nover2p1
    ! Condition for small values
    increment = SUM(CVT % HorizEV1(k,1:nlevs)) * ConditionFudge
    IF (increment < small) increment = small
    CVT % HorizEV1(k,1:nlevs) = CVT % HorizEV1(k,1:nlevs) + increment

    increment = SUM(CVT % HorizEV2(k,1:nlevs)) * ConditionFudge
    IF (increment < small) increment = small
    CVT % HorizEV2(k,1:nlevs) = CVT % HorizEV2(k,1:nlevs) + increment

    increment = SUM(CVT % HorizEV3(k,1:nlevs)) * ConditionFudge
    IF (increment < small) increment = small
    CVT % HorizEV3(k,1:nlevs) = CVT % HorizEV3(k,1:nlevs) + increment

    increment = SUM(CVT % HorizEV4(k,1:nlevs)) * ConditionFudge
    IF (increment < small) increment = small
    CVT % HorizEV4(k,1:nlevs) = CVT % HorizEV4(k,1:nlevs) + increment

    increment = SUM(CVT % HorizEV5(k,1:nlevs)) * ConditionFudge
    IF (increment < small) increment = small
    CVT % HorizEV5(k,1:nlevs) = CVT % HorizEV5(k,1:nlevs) + increment

    increment = SUM(CVT % HorizEV6(k,1:nlevs)) * ConditionFudge
    IF (increment < small) increment = small
    CVT % HorizEV6(k,1:nlevs) = CVT % HorizEV6(k,1:nlevs) + increment
