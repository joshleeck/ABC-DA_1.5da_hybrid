...
CHARACTER(LEN=320)          :: ABC_init_file
...
CVT % CVT_order         = CVT_order
CVT % CVT_param_opt_gb  = CVT_param_opt_gb
CVT % CVT_param_opt_hb  = CVT_param_opt_hb
CVT % CVT_param_opt_ab  = CVT_param_opt_ab
CVT % CVT_param_opt_reg = CVT_param_opt_reg
CVT % CVT_vert_opt_sym  = CVT_vert_opt_sym
CVT % CVT_stddev_opt    = CVT_stddev_opt
...
CASE (1)
    ...
    DO ens = 1, Nens
      ...
      DO mem = 1, NEnsMems
        ...
        DO lat = 1, Nlats
          ...
          !CALL Write_state_2d (testfile, ABC_data_a, dims, 1, 0, 0, .TRUE.)
          ...
          CALL Write_state_2d (ABCfile, ABC_data_a, dims, 1, 0, 0, .TRUE.)
  ...
  ELSE
    ...
    ELSE
      ! Not producing any states
      PRINT *, 'Not scheduled to produce any states, only producing prototype cov file'

      ! Need to know longs_v and half_levs to output to cov file
      ! Do this by reading-in an existing ensemble member
      CALL Initialise_model_vars (ABC_data_a, .FALSE.)

      CALL Initialise_dims (dims)
      ABC_init_file = TRIM(datadirABC_in)  // '/' // TRIM(init_ABC_file)
      PRINT *, 'Reading state ', TRIM(ABC_init_file), ' for longs and levs'
      CALL Read_state_2d (ABC_init_file, ABC_data_a, dims, -1, .TRUE.)
  ...
  CALL Deallocate_model_vars (ABC_data_b)
...
CASE (2)
  ...
  IF (Nens > 0) THEN
    ...
    DO ens = 1, Nens
      ...
      CALL Write_state_2d (meanfile, ABC_mean, dims, 1, 0, 0, .TRUE.)
      PRINT *, '-- done'
...
CASE (5)
  ...
  CALL Initialise_CVs (ControlVar_1, .FALSE.)
  ...
  CALL Deallocate_CVs (ControlVar)
  CALL Deallocate_CVs (ControlVar_mean)
  CALL Deallocate_CVs (ControlVar_1)

END SELECT

CALL Deallocate_dims (dims)
CALL Deallocate_model_vars (ABC_data_a)
