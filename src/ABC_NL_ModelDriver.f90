...
CALL Initialise_model_vars (StateA, .FALSE.)
CALL Initialise_model_vars (StateB, .FALSE.)
...
PRINT*, 'dumpspacing            ', dumpspacing
IF (ndumps > 0) PRINT*, 'Output times every     ', dt*ntimesteps/(3600*ndumps),' hrs'
...
CALL Energy(StateA, dims)
CALL Effective_buoyancy(StateA, dims)
CALL Boundaries (StateA, set_beff=.TRUE.)
CALL Write_state_2d (fileout, StateA, dims, ndumps+1, 0, dumpspacing, .TRUE.)
...
DO t = 1, ntimesteps
 ! PRINT*,' > ', t, ' of ', ntimesteps
  ...
  IF ((t/dumpspacing)*dumpspacing == t) THEN
    ...
    CALL Write_state_2d (fileout, StateB, dims, ndumps+1, t/dumpspacing, dumpspacing, .TRUE.)
  ENDIF
...
CALL Deallocate_model_vars (StateA)
CALL Deallocate_model_vars (StateB)
