...
REAL(ZREAL8), ALLOCATABLE      :: u_at_b(:,:), w_at_u(:,:)
REAL(ZREAL8), ALLOCATABLE      :: u_at_v(:,:), w_at_v(:,:)
...
CALL Initialise_model_vars (adjust, .FALSE.)
CALL Initialise_model_vars (adjust_p1, .FALSE.)
ALLOCATE (u_at_b(1:nlongs,1:nlevs))
ALLOCATE (w_at_u(1:nlongs,1:nlevs))
ALLOCATE (u_at_v(1:nlongs,1:nlevs))
ALLOCATE (w_at_v(1:nlongs,1:nlevs))
...
! Tidy up
CALL Deallocate_Averages (aves)
CALL Deallocate_model_vars (adjust)
CALL Deallocate_model_vars (adjust_p1)
DEALLOCATE (u_at_b, w_at_u, u_at_v, w_at_v)
