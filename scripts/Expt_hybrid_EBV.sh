...
PLOT_ENS=1
#ENS_BOUND="(5,15,86,96)"
ENS_BOUND="(25,35,177,187)"
...
EOF
  ...
  if [ $CALCS == 1 ]; then
    echo "  Producing ensemble members using EBV ..." >> $BASE_DIR/SuiteOut
    $CODE_DIR/Master_MakeEns.out > stdout 2> stderr
    mv stdout MakeEns_stdout
    mv stderr MakeEns_stderr
    echo "  ... done" >> $BASE_DIR/SuiteOut
...
EOF
  ...
  if [ $PLOT_ENS == 0 ]; then
    echo " Plotting/processing ..." >> $BASE_DIR/SuiteOut
    python $PLOT_CODE_DIR/PlotAssimDiagsEns.py $CYCLE_DIR $N_ENS $ENS_BOUND
    echo "  ... done"
  fi

  # Compute ensemble variance for each cycle
  if [ $PLOT_ENS == 0 ]; then
    ...
    #echo ${arrFiles[@]}
    #cdo ensvar1 ${arrFiles[@]} ABC_bg_ensvar1.nc
    cdo ensvar ${arrFiles[@]} ABC_bg_ensvar.nc
  fi
...
if [ $PLOT_ENS == 0 ]; then  
  ...
  python $PLOT_CODE_DIR/PlotMultiCycleErrorsEns.py $BASE_DIR $N_ENS $ENS_BOUND
  echo "... done" >> $BASE_DIR/SuiteOut
fi
...
if [ $PLOT_ENS == 1 ]; then
  ...
  python $PLOT_CODE_DIR/PlotMultiCycleSpreadError.py $BASE_DIR $ENS_BOUND
  echo "... done" >> $BASE_DIR/SuiteOut
fi
