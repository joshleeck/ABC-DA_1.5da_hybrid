...
# ===================================================================
# ===================================================================
def plot_1_field_diff_custom (filename1, filename2, variable, x_grid_name, z_grid_name, prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple):
  nc_file1 = Dataset(filename1)
  field1   = nc_file1.variables[variable][:,:,:]
  nc_file2 = Dataset(filename2)
  field2   = nc_file2.variables[variable][:,:,:]
  ...
  RMSd      = []
  Meand     = []
  z1, z2, x1, x2 = bound_tuple
  for t in range(ntimes):
    print ('Dealing with u at time', t)
    # Various averages of each field separately
    # Gridpoint mean (custom) instead of field mean if required
    #RMS1.append (np.sqrt(np.mean(field1[t][:][:] * field1[t][:][:])))
    #Mean1.append(np.mean(field1[t][:][:]))
    RMS1.append (np.sqrt(np.mean(field1[t,z1:z2,x1:x2] * field1[t,z1:z2,x1:x2])))
    Mean1.append(np.mean(field1[t,z1:z2,x1:x2]))
    #RMS2.append (np.sqrt(np.mean(field2[t][:][:] * field2[t][:][:])))
    #Mean2.append(np.mean(field2[t][:][:]))
    RMS2.append (np.sqrt(np.mean(field2[t,z1:z2,x1:x2] * field2[t,z1:z2,x1:x2])))
    Mean2.append(np.mean(field2[t,z1:z2,x1:x2]))
    # Deal with the differences, field2 - field1
    #minlev = min_field2d(field[t][:][:])
    #maxlev = max_field2d(field[t][:][:])
    minlev = min_field2d(field[t,z1:z2,x1:x2])
    maxlev = max_field2d(field[t,z1:z2,x1:x2])

    print ('Min value of this field ', minlev)
    print ('Max value of this field ', maxlev)
    # Compute the RMS and mean of the difference
    #RMSd.append (np.sqrt(np.mean(field[t][:][:] * field[t][:][:])))
    RMSd.append (np.sqrt(np.mean(field[t,z1:z2,x1:x2] * field[t,z1:z2,x1:x2])))
    # Gridpoint mean (custom) instead of field mean if required
    #Meand.append(np.mean(field[t][:][:]))
    Meand.append(np.mean(field[t,z1:z2,x1:x2]))
...
# ===================================================================
# ===================================================================
def plot_diff_fields_custom (filename1, filename2, prefix, output_type, html_file, plotdir, C_param, f, TimeOutput, bound_tuple):
  ...
  # Deal with the zonal wind
  # -------------------------------------------------------------------
  RMS1, Mean1, RMS2, Mean2, RMSd, Meand = plot_1_field_diff_custom (filename1, filename2, 'u', 'longs_u', 'half_level', prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple)
  # Append the time sequence to the lists that will contain all quantities
  QuantityNames.append('u')
  Mean_file1.append(Mean1)
  RMS_file1.append(RMS1)
  ...
  # Deal with the meridional wind
  # -------------------------------------------------------------------
  RMS1, Mean1, RMS2, Mean2, RMSd, Meand = plot_1_field_diff_custom (filename1, filename2, 'v', 'longs_v', 'half_level', prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple)
  # Append the time sequence to the lists that will contain all quantities
  QuantityNames.append('v')
  Mean_file1.append(Mean1)
  RMS_file1.append(RMS1)
  ...
  # Deal with the vertical wind
  # -------------------------------------------------------------------
  RMS1, Mean1, RMS2, Mean2, RMSd, Meand = plot_1_field_diff_custom (filename1, filename2, 'w', 'longs_v', 'full_level', prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple)
  # Append the time sequence to the lists that will contain all quantities
  QuantityNames.append('w')
  Mean_file1.append(Mean1)
  RMS_file1.append(RMS1)
  ...
  # Deal with the r_prime
  # -------------------------------------------------------------------
  RMS1, Mean1, RMS2, Mean2, RMSd, Meand = plot_1_field_diff_custom (filename1, filename2, 'r_prime', 'longs_v', 'half_level', prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple)
  # Append the time sequence to the lists that will contain all quantities
  QuantityNames.append('r_prime')
  Mean_file1.append(Mean1)
  RMS_file1.append(RMS1)
  ...
  # Deal with b_prime
  # -------------------------------------------------------------------
  RMS1, Mean1, RMS2, Mean2, RMSd, Meand = plot_1_field_diff_custom (filename1, filename2, 'b_prime', 'longs_v', 'full_level', prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple)
  # Append the time sequence to the lists that will contain all quantities
  QuantityNames.append('b_prime')
  Mean_file1.append(Mean1)
  RMS_file1.append(RMS1)
  ...
  # Deal with the tracer
  # -------------------------------------------------------------------
  RMS1, Mean1, RMS2, Mean2, RMSd, Meand = plot_1_field_diff_custom (filename1, filename2, 'tracer', 'longs_v', 'half_level', prefix, filesuffix, output_type, html_file, plotdir, TimeOutput, bound_tuple)
  # Append the time sequence to the lists that will contain all quantities
  QuantityNames.append('tracer')
  Mean_file1.append(Mean1)
  RMS_file1.append(RMS1)
...
# ===================================================================
# ===================================================================
def plot_scalar_time_seq_custom (time, cycle_bound_times, quantities, code, TRUTH_mean, DATA_mean, FREE_mean, TRUTH_RMS, DATA_RMS, FREE_RMS, DATA_RMSE, FREE_RMSE, output_type, html_file, plot_dir):
  # Subroutine to plot time sequencies of mean_diff, and rms_diff
  # These structures are lists of sublists.
  # Each sublist is the time sequence for each quantity
  # The scale-resolved balance diagnostics have only rms_diff entries
  ...
  if 'fbg_truth' in code:
    ...
    for cycle in cycle_bound_times:
      plt.axvline(x=cycle, color='yellow')
    ax.plot(time[:], TRUTH_RMS[0][:],  linewidth=1, ls='solid', color='red', label='u RMS (truth)')
    ax.plot(time[:], TRUTH_RMS[1][:],  linewidth=1, ls='solid', color='blue', label='v RMS (truth)')
    ax.plot(time[:], TRUTH_RMS[2][:],  linewidth=1, ls='solid', color='green', label='w RMS (truth)')
    #ax.plot(time[:], TRUTH_mean[0][:], linewidth=2, ls='solid', color='red', label='mean u (truth)')
    #ax.plot(time[:], TRUTH_mean[1][:], linewidth=2, ls='solid', color='blue', label='mean v (truth)')
    #ax.plot(time[:], TRUTH_mean[2][:], linewidth=2, ls='solid', color='green', label='mean w (truth)')
    if '_fbg' in code:
      ax.plot(cycle_bound_times[:-1], FREE_RMS[0][:], linewidth=2, ls='dotted', color='red', label='u RMS (free)')
      ax.plot(cycle_bound_times[:-1], FREE_RMS[1][:], linewidth=2, ls='dotted', color='blue', label='v RMS (free)')
      ax.plot(cycle_bound_times[:-1], FREE_RMS[2][:], linewidth=2, ls='dotted', color='green', label='w RMS (free)')
    ax.plot(time[:], DATA_RMS[0][:], linewidth=2, ls='dashed', color='red', label='u RMS')
    ax.plot(time[:], DATA_RMS[1][:], linewidth=2, ls='dashed', color='blue', label='v RMS')
    ax.plot(time[:], DATA_RMS[2][:], linewidth=2, ls='dashed', color='green', label='w RMS')
    ax.legend(loc='best') #(loc='upper right')
    if 'anal_fbg' in code:
      graphics_file_name = 'Plots/Wind_anal_mixed' + filesuffix
      plt.title('RMS anal values of wind components')
      if '_ens' in code:
        graphics_file_name = 'Plots/Wind_anal_mixed_' + code.split('_ens')[-1]
    elif 'bg_fbg' in code:
      graphics_file_name = 'Plots/Wind_bg_mixed' + filesuffix
      plt.title('RMS bg values of wind components')
    #plt.show()             # Comment out to not display plot on the screen
    plt.savefig(plot_dir + '/' + graphics_file_name)  # Comment out to not save the file
    plt.close('all')
    if (output_type == 'web'):
    ...
    for cycle in cycle_bound_times:
      plt.axvline(x=cycle, color='yellow')
    ax.plot(time[:], TRUTH_RMS[3][:], linewidth=1, ls='solid', color='red', label='RMS (truth)')
    #ax.plot(time[:], TRUTH_mean[3][:], linewidth=2, ls='solid', color='red', label='mean (truth)')
    if '_fbg' in code:
      ax.plot(cycle_bound_times[:-1], FREE_RMS[3][:], linewidth=2, ls='dotted', color='red', label='RMS (free)')
    ax.plot(time[:], DATA_RMS[3][:], linewidth=2, ls='dashed', color='red', label='RMS')
    ax.legend(loc='best') #(loc='upper right')
    if 'anal_fbg' in code:
      plt.title('RMS anal values of rp')
      graphics_file_name = 'Plots/rp_anal_mixed' + filesuffix
      if '_ens' in code:
        graphics_file_name = 'Plots/rp_anal_mixed_' + code.split('_ens')[-1]
    elif 'bg_fbg' in code:
      plt.title('RMS bg values of rp')
      graphics_file_name = 'Plots/rp_bg_mixed' + filesuffix
    #plt.show()             # Comment out to not display plot on the screen
    plt.savefig(plot_dir + '/' + graphics_file_name)  # Comment out to not save the file
    plt.close('all')
    if (output_type == 'web'):
    ...
    fig, ax = plt.subplots()
    ax.set_xlabel('time (h)')
    ax.set_ylabel('RMS (m/s2)')
    fig.set_size_inches(14.0, 7.0)
    for cycle in cycle_bound_times:
      plt.axvline(x=cycle, color='yellow')
    ax.plot(time[:], TRUTH_RMS[4][:], linewidth=1, ls='solid', color='blue', label='RMS (truth)')
    #ax.plot(time[:], TRUTH_mean[4][:], linewidth=2, ls='solid', color='blue', label='mean (truth)')
    if '_fbg' in code:
      ax.plot(cycle_bound_times[:-1], FREE_RMS[4][:], linewidth=2, ls='dotted', color='blue', label='RMS (free)')
    ax.plot(time[:], DATA_RMS[4][:], linewidth=2, ls='dashed', color='blue', label='RMS')
    ax.legend(loc='best') #(loc='upper right')
    if 'anal_fbg' in code:
      plt.title('RMS anal values of buoyancy')
      graphics_file_name = 'Plots/bp_anal_mixed' + filesuffix
      if '_ens' in code:
        graphics_file_name = 'Plots/bp_anal_mixed_' + code.split('_ens')[-1]
    elif 'bg_fbg' in code:
      plt.title('RMS bg values of buoyancy')
      graphics_file_name = 'Plots/bp_bg_mixed' + filesuffix
    #plt.show()             # Comment out to not display plot on the screen
    plt.savefig(plot_dir + '/' + graphics_file_name)  # Comment out to not save the file
    plt.close('all')
    if (output_type == 'web'):
