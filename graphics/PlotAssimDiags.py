...
#  11. Imbalance diagnostics with time (background, analysis, and truth)
...
if PlotIts:
  ...
  while (line != ''):
    ...
    # Append dummy values just for second outer loop (eventually are discarded below)
    if (int(temp[0]) == 2):
      innerloop.append (0)
      Jb.append (0.0)
      Jo.append (0.0)
      ...
      Je.append (float(temp[4]))
      J.append (float(temp[5]))
      grad.append (float(temp[6].replace('*','')))
    #if (int(temp[1]) == 0):
    #  # Energy output only at the start of the outer loop
    #  KE.append (0.0)   #KE.append (float(temp[6]))
    #  BE.append (0.0)   #BE.append (float(temp[7]))
    #  EE.append (0.0)   #EE.append (float(temp[8]))
    #  TE.append (0.0)   #TE.append (float(temp[9]))
    line = filediags.readline()
  ...
  ax.plot(totalit[:], Je[:], linewidth=2, color='green', label='Je')
